...
const registerSeller = Joi.object({
    ...
    password : Joi.string().min(6).message('password minimal 6 karakter').pattern(new RegExp('^((?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,}$|penjual)')).message('Silahkan gunakan kombinasi karakter besar kecil, angka dan symbol').required(),
})
...
const updateSeller = Joi.object({
    ...
    password : Joi.string().min(6).message('password minimal 6 karakter').pattern(new RegExp('^((?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,}$|penjual)')).message('Silahkan gunakan kombinasi karakter besar kecil, angka dan symbol'),
})
